...
export default class InventoryList extends React.Component {
  ...
  render () {
    ...
    const filteredList = this.state.inventoryList.filter(item =>
      item.itemName.toLowerCase().includes(this.state.searchTerm.toLowerCase())
    );
...
/>
        ...
        <ul className="inventory__list">
          {  (filteredList.length === 0)? <h4 style={{'textAlign':"center",'justifyContent':'center'}}>🧐No result Found! 🤔</h4>:       filteredList.map((item) => (
